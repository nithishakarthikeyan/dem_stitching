...
def main():
    ...
    gscript.run_command("g.region", raster=[input_A, input_B])

    print("Running mapcalc for overlap mask...")
    ...
    # Step 4: Compute post-shift diff
    gscript.mapcalc(
    ...
    # Step 5: Stitch final output
    gscript.mapcalc(
        f'"{tmp_result}" = if(!isnull("{input_A}"), "{tmp_a_shifted}", "{input_B}")',
        overwrite=True
    )

    # Step 6: Compute stddev
    stats = gscript.parse_command('r.univar', map=tmp_postshift_diff, flags='ge')
